...
class _MySideBar extends StatelessWidget {
  const _MySideBar(this.data);
  final List<MyDashboardModel> data;

  @override
  Widget build(BuildContext context) {
    return SidebarX(
      controller: companyUniversalController.sideBarController,
      ...
      headerBuilder: (context, extended) {
        return InkWell(
          onTap: () {
            companyUniversalController.sideBarController.selectIndex(0);
            companyUniversalController.setTabIndex = 0;
            companyUniversalController.scaffoldKey.currentState?.closeDrawer();
          },
          child: SizedBox(
            height: 100,
      ...
      items: data
          .map(
            (e) => SidebarXItem(
              ...
              label: e.title,
              onTap: () {
                companyUniversalController.setTabIndex = data.indexOf(e);
                companyUniversalController.scaffoldKey.currentState
                    ?.closeDrawer();
              },
            ),
          )
